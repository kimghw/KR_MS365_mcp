...
## 지원 서버 (outlook + calendar)

| 서버 | HTTP 포트 | STDIO 스크립트 | Stream 스크립트 |
|---|---|---|---|
| `outlook` | 8091 (env `MCP_OUTLOOK_PORT`) | `mcp_outlook/mcp_server/server_stdio.py` | `mcp_outlook/mcp_server/server_stream.py` |
| `calendar` | 8002 (env `MCP_CALENDAR_PORT`) | `mcp_calendar/mcp_server/server_stdio.py` | `mcp_calendar/mcp_server/server_stream.py` |

**공유 자원:**
- `.env` — 두 서버가 같은 Azure 자격증명 사용
- `session/auth_manager.py` + `database/auth.db` — 1회 인증 → 두 서버 공통
...
**setup_ms365 스킬은 항상 `-s user` 사용** — outlook/calendar는 디렉토리 무관하게 항상 보여야 하므로.
...
엔트리 스키마 (outlook + calendar 동시 등록 예):
...
```json
{
  "mcpServers": {
    ...
    "calendar": {
      "command": "c:\\Users\\USER\\KR_MS365_mcp\\venv\\Scripts\\python.exe",
      "args": ["c:\\Users\\USER\\KR_MS365_mcp\\mcp_calendar\\mcp_server\\server_stdio.py"],
      "env": {
        "PYTHONPATH": "c:\\Users\\USER\\KR_MS365_mcp",
        "PYTHONUTF8": "1",
        "PYTHONIOENCODING": "utf-8"
...
| 8091 | MCP HTTP 서버 (`server_stream.py`) | 환경변수 `MCP_SERVER_PORT` |
