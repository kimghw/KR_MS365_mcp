...
```bash
SERVER_BASE=http://localhost:8091    # 검사할 서버 URL
...
```bash
SERVER_BASE=http://localhost:8091    # 검사 대상
...
7. **포트 / 환경변수** — `MCP_SERVER_PORT` 환경변수 우선, 없으면 서버별 기본값(outlook 8091, calendar 8002 등).
...
`.claude/skills/setup_ms365/scripts/`에 `streamable_http_probe.sh`(또는 `.py`) 추가를 권장. 인자로 `SERVER_BASE`(예: `http://localhost:8091`)를 받고 8개 probe를 차례로 돌려 표 형태로 합/불 출력. 본 체크리스트의 curl 블록을 그대로 옮기면 됨.
