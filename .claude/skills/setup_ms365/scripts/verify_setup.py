...
# 서버별 정의 (이름 → HTTP 포트 + STDIO 스크립트 경로)
SERVERS = {
    "outlook": {
        "http_port": int(os.environ.get("MCP_OUTLOOK_PORT", "8091")),
        "stdio_script": PROJECT_ROOT / "mcp_outlook" / "mcp_server" / "server_stdio.py",
        "stream_script": PROJECT_ROOT / "mcp_outlook" / "mcp_server" / "server_stream.py",
    },
    "calendar": {
        "http_port": int(os.environ.get("MCP_CALENDAR_PORT", "8002")),
        "stdio_script": PROJECT_ROOT / "mcp_calendar" / "mcp_server" / "server_stdio.py",
        "stream_script": PROJECT_ROOT / "mcp_calendar" / "mcp_server" / "server_stream.py",
    },
}
