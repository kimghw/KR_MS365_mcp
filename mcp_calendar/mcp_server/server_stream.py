...
def build_mcp_server() -> MCPServer:
    ...
    server: MCPServer = MCPServer(name="calendar", version="1.0.0")
    ...
    # validate_input=False — existing handlers accept the YAML-converted
    # enabled/disabled string-enum form; matches server_stdio.py behavior.
        ...
        async with session_manager.run():
            ...
            if hasattr(calendar_service, "initialize"):
                try:
                    ...
                    logger.warning(f"CalendarService initialize() failed: {e}")
            logger.info(f"Calendar MCP Streamable HTTP server ready with {len(MCP_TOOLS)} tools")
            yield
