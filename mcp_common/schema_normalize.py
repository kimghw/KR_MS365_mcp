...
2026-08-12 코드 생성 폐지로 **사본이 필요 없어졌다.** 이제 소비자는
`mcp_common.param_spec` 하나뿐이고, 그 모듈이 기동 시 `spec/param_spec/<도메인>.yaml`
에서 `inputSchema` 를 파생시킬 때 이 규칙을 적용한다. 이 모듈이 유일한 원본이다
(spec/spec_도구정의.md).
